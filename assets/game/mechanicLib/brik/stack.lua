...
function stack.switch(P)
    ...
    if not md.stack_enabled then
        ...
        local lines=mechLib.brik.clearRule.line.getFill(P)
        if lines then
            P:doClear(lines)
            P:freshGhost()
        end
...
function stack.event_afterLock_noFall(P)
    if P.modeData.stack_enabled then
        ...
        for y=1,F:getHeight() do
            if not md.stack_lineList[y] and mechLib.brik.clearRule.line.isFill(P,y) then
                table.insert(list,y)
...
function stack.event_drawOnPlayer(P)
    ...
    if md.stack_enabled and md.stack_lines>0 then
        ...
        GC.strokePrint('full',2,COLOR.lD,COLOR.L,md.stack_lines,0,-fontSize*.5,nil,'center')
        GC.pop()
    end
end
