    ...
    elseif key=='space' then
        ...
        elseif state==3 then -- play
            ...
            if hit[c]==0 then
                hit[c]=1
                count=count+1
                FMOD.playEffect(side<26 and 'ren_'..rnd(5) or 'ren_'..rnd(6,11))
                if count>=12 then
                    FMOD.playEffect('ren_mega',{volume=(count-11)/15})
                end
                if count==side then
                    state=1
                    FMOD.playEffect('spin_0')
                else
...
function scene.draw()
    ...
    if state==0 then
        gc.setColor(0,0,0,1-timer/50)
        setFont(30)
        mStr(MOBILE and "Touch to Start" or "Press space to Start",800,630)
    else
...
scene.widgetList={
    WIDGET.new{type='button',x=1140,y=60,w=170,h=80,color='D',sound_trigger='button_back',fontSize=60,text=CHAR.icon.back,code=WIDGET.c_backScn()},
}
