...
function scene.enter()
    ...
    brikMap:reset()
    brikMap:setFullVersion(
        PROGRESS.getModeState('brik_stdMap','sprint_40')>0 or
        PROGRESS.getModeState('brik_stdMap','marathon')>0 or
        PROGRESS.getModeState('brik_stdMap','dig_practice')>0
    )
    panel:setSel(false)
...
function scene.mouseMove(x,y,dx,dy)
    ...
    if love.mouse.isDown(1) then
        brikMap:moveCam(dx,dy)
    else
        x,y=SCR.xOy:transformPoint(x,y)
        brikMap:mouseMove(x,y)
    end
end
...
local function sysAction(action)
    if action=='select' then
        panel:setSel(brikMap:keyboardSelect())
    elseif action=='back' then
        if not PROGRESS.getModeUnlocked('acry_wip') and not PROGRESS.getModeUnlocked('gela_wip') and PROGRESS.getModeUnlocked('brik_stdMap') then
            SCN._pop()
        end
        SCN.back('fadeHeader')
    end
...
function scene.update(dt)
    brikMap:update(dt)
    brikMap:keyboardMove(SCR.xOy:transformPoint(800,600))
    panel:update(dt)
end
