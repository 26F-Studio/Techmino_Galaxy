...
scene.widgetList={
    ...
    WIDGET.new{name='1',type='button', pos={0,0},x=400, y=910,w=160, h=80,cornerR=10,fontSize=40, text=LANG'setting_test',           code=playExterior'mino/exterior/test'},

    -- Audio
    WIDGET.new{name='2',type='slider_fill',pos={0,0},x=340, y=220,w=650, fontSize=40,text=LANG'setting_mainVol', widthLimit=380, disp=TABLE.func_getVal(SETTINGS.system,'mainVol'), code=TABLE.func_setVal(SETTINGS.system,'mainVol')},
    ...
    WIDGET.new{name='3',type='slider',     pos={0,0},x=340, y=540,w=250,     text=LANG'setting_msaa',        widthLimit=380,axis={0,4,1},                 disp=function() return SETTINGS.system.msaa==0 and 0 or math.log(SETTINGS.system.msaa,2) end, valueShow=function(S) return (S.disp()==0 and 0 or 2^S.disp()).."x" end,  code=function(v) SETTINGS.system.msaa=v==0 and 0 or 2^v; saveSettings(); if TASK.lock('warnMessage',6.26) then MES.new('warn',Text.setting_needRestart,6.26) end end},
