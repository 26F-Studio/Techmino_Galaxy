...
function scene.draw()
    ...
    if state==0 then
        setFont(40)
        mStr(MOBILE and "Touch to Start" or "Press space to Start",800,400)
        mStr("Press key on the same side when block appear!",800,500)
        if ct>0 then
            setFont(100)
            gc.setColor(1,1,1,ct/30)
            mStr("REFLECT",800,140)
        end
    elseif state==1 then
        gc.setColor(.2,.7,.4,math.min((60-ct)/10,ct/10)*.8)
        gc.arc('fill',640,360,260,-1.5708,-1.5708+(ct/60)*MATH.tau)
