...
local function tapBoard(x,y,key)
    if state<2 then
        if not key then
            ...
            x,y=int((x-SCR.w0/2)/160/scale)+3,int((y-SCR.h0/2)/160/scale)+3
        end
        local b=board
        local moves=0
        if cx==x then
...
scene.widgetList={
    WIDGET.new{type='button',  pos={0,0},x=160, y=100,w=180,h=100,color='lG',fontSize=60,text=CHAR.icon.retry_spin,code=WIDGET.c_pressKey'space'},
    colorSelector,
