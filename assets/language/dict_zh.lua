...
规则名，指使用所有方块进行卡块消都会得到奖励，作用为强调规则不是另一个常见规则T-Only，该规则只有T方块的回旋操作能获得奖励。
...
# spike
@ title 爆发（Spike）
指短时间内打出大量的攻击，因为这会在攻击-时间的曲线图表上显示为尖刺（Spike的本义）。
...
# his_sequence
@ title His出块
History（历史记录），一类序列生成方式，具体的类型例如His4 Roll6（h4r6），效果是在随机生成新的方块时，先在七个块中直接随机，然后对比前4个生成的方块，如果有相同的就重新随机，直到和最近四个都不同或者重试了6次。
His出块是相比纯随机出块的一大改进，大大减小了连续出太多SZ（洪水）和很久没有I（干旱）的概率。

# bag7_sequence
@ title Bag7出块
包循环，一种常见的序列生成方式。从开局起，每七个块包含七种形状各一个，只随机打乱顺序。这种序列生成方式彻底避免了很久不出某种块和某种块来得特别多的情况。
...
# hypertap
@ title 超连点（Hypertap）
在ASD很慢或其他不适合使用长按移动的情况下，快速震动手指以实现比长按更快速+灵活的高速单点移动。
